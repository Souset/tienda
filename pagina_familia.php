    ...
    <div class="wrapper">
        ...
        <div class="content-wrapper">
            ...
            <section class="content-header">
                <h1>
                    <a href="pagina_familia.php?familia=<?php echo $familia[0]["id"] ?>">
                        <?php echo $familia[0]["familia"] ?>
                    </a>
                </h1>
                <ol class="breadcrumb">
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            <li class="<?php if ($pagina == 1) { echo "disabled"; } ?>">
                                <a href="<?php if ($pagina > 1) { echo "pagina_familia.php?familia=$familia_id&pagina=" . ($pagina - 1); } else { echo "#"; } ?>" aria-label="Anterior">
                            ...
                            <?php if ($total_paginas < 10) { ?>
                                <?php for ($j=1; $j<=$total_paginas; $j++) { ?>
                                    <li class="<?php if ($pagina == $j) { echo "active"; } ?>">
                                        <a href="<?php if ($pagina != $j) { echo "pagina_familia.php?familia=$familia_id&pagina=" . $j; } else { echo "#"; } ?>">
                                            <?php echo $j?>
                                        </a>
                                    </li>
                                <?php } ?>
                            ...
                            <li class="<?php if ($pagina == $total_paginas) { echo "disabled"; } ?>">
                                <a href="<?php if ($pagina < $total_paginas) { echo "pagina_familia.php?familia=$familia_id&pagina=" . ($pagina + 1); } else { echo "#"; } ?>" aria-label="Siguiente">
                                    <span aria-hidden="true">&raquo;</span>
                                </a>
                            </li>
                        </ul>
                        <div>
                            <form action="javascript:ir_a_pagina(<?php echo $total_paginas . ", " . $familia_id ?>)">
                                <label for="pagina">Pagina </label>
                                <input id="pagina" name="pagina" value="<?php echo $pagina ?>" type="text" autocomplete="off">
                                <label>de <?php echo $total_paginas ?></label>
                                <button id="pagina_boton" type="button" onclick="ir_a_pagina(<?php echo $total_paginas . ", " . $familia_id ?>)">Ir</button>
                            </form>
                        </div>
                    </nav>
                </div>
...
      
    ...
    //  FORMULARIO PARA LA PAGINACION
        function ir_a_pagina(total_paginas, familia) {
